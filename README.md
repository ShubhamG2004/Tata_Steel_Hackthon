...
This repository provides a reproducible baseline for the "Defect Detection in Hot Rolling" challenge. It trains a threshold-tuned gradient boosting model on `dataset/train.csv` and produces a submission file `expected_submission.csv` for the test set `dataset/test.csv`.

Structure:
- `dataset/` — contains the provided `train.csv`, `test.csv`, and `sample_submission.csv` files.
